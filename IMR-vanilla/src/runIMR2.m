...
if whetherToSolveP > 0
    for tempk = 1:length(expts)
        ...
        P_upperlimit = 400; P_lowerlimit = 100; % JY!!! pay attention to the assigned limit values of P.
        
        while (abs(P_upperlimit-P_lowerlimit)/P_upperlimit > 1e-3) && (P0Iter < 100)
            P0Iter = P0Iter+1;
            % ====== IMR decoupled system solver + tune P_initial ======
            tic;
            [t2_num, P, T, C, tdel, Tdel, Cdel] = funIMRsolver2_DecouplePCT(...
...
if whetherToSolveP == 0
    ...
    for tempk = 1:length(expts)
        ...
        options = optimset('Display','iter','PlotFcns','optimplotfval','TolX',1e-2);
        ...
        for i = 1 :length(G_ooms)
            for j = 1 :length(mu_ooms) %mu
                for k = 1 :length(G1_ooms) % for j = 1:length(mu_ooms)
                    %for l = 1 % 1:length(lambda_nu_ooms) % for i=1:length(G_ooms)
                    ...
                    [tempval1,tempval2] = funIMR2LSQErr(model,matPropVar,tspanLocsMaxP(1:PickNo),pksMaxR(1:PickNo),tspanLocsMaxR(1:PickNo+1),NT/10,NTM,...
                        Pext_type,Pext_Amp_Freq,disptime,Tgrad,Tmgrad,Cgrad,Dim,comp,Pfit_pp,Pfit_pp_d1,eqR,timestep,timetolList(expt,:),Rfit_pp);
                    LSQErr{i,j,k} = tempval1;
                    LSQErrNo{i,j,k} = tempval2;
                    %end
